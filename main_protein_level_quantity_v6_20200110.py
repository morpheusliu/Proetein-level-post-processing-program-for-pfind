#这个版本的unique peptide定义是看每个谱中存在几个leading ac 如果只存在一个leading ac 就是unique peptide，不管有几个sameset，如果存在多个leading ac 就不是unique的.
#这一个版本在v2的基础上引入了pfind protein group的数据 借助这个数据帮助我们确定dataset中最有可能出现的蛋白ac 取代以往的把第一个ac作为leading ac的盲目做法。
#这个版本可以做多个modification和单个modification分析，相关语句已经于20181226进行更正
#可用于apex的数据分析。
#算法为ptm规则，没有选取unique 匹配的肽段。
#如果一个肽段含有多个modification则把这个肽段的值按照modification的数量复制，每个里面只含有一个modification位点，然后分别按位点进行后面的计算。即含有多个modification位点的肽段项会被重复计算。
#2019年12月5日对ratio的计算进行更改，之前的输出结果为平均值，现在改为中值
...
mc=input("Please enter the number of modification in each spectrum(a for only modification & b for multi-modification):")
pt=input("Please make sure the peptide type (y for ptm mode & n for unique peptide mode):")
ru=input("Do you need real uniquepeptide (real uniquepeptide is a peptide with only one ac)(y for yes and n for no):")	#这个参数会在第一次数据筛选的时候就使用
print(calculation)
import glob
...
for file_name in file_list:
	...
	while True:
		...
		if arry_raw_data[10]<=float(intscore):	#interference score的筛选值 根据需要进行修改
			if ru=='y':
				if arry_raw_data[6].count('|')==2:	#判断这一行数据是不是对应了一个ac，这个判断比较粗糙，可能包含了反向库匹配和污染库匹配，但是在后续的计算中会去除
	...
	while True:
		...
		for acs in arry2_arry_data1:
			if acs.find("REV_")==0 or acs.find("CON_")==0:	#检查肽段对应的蛋白id中是否包含con_和rev_，如果有则去除这一行的数据，但是结果中可能包含的id为空值的可能，经过这个筛选，输出文件data2中将不会存在包含污染和反向库的项
				xcount=xcount+1
		if xcount>0:
			continue
		if mc=="a":	#不包含反向库和污染项的数据行进行参数a和b的筛选，但是包含的id中可能含有空项
		...
		if len(arry_data_protein)>=2:
			try:
				...
				if 'SameSet'==arry_data_protein[1]:
					if arry_data_protein[2].find("REV_")==0 or arry_data_protein[2].find("CON_")==0:
						pass
					else:
						nn+=1
						if nn<=20:	#将seameset的数量限制在20个以内
							...
							file_result.close()
				continue
			if int(arry_data_protein[0])>0 and not arry_data_protein[1].find('REV_')==0 and not arry_data_protein[1].find('CON_')==0:
	...
	while True:
		...
		for acsssss in arry_tem2:
			if os.path.exists('data_protein_group/'+acsssss):
				nnn+=1
		if pt=='n' and nnn>1:	#进行unique peptide筛选
			continue
					...
					while True:
						...
						while True:
							...
							if not site+1:
								break
							if int(sitess)==0:
								final_site=site+int(sitess)+1
							else:
								final_site=site+int(sitess)
							arry_all_site.append(str(final_site))
							sequence=sequence[:site]
					file_library.close()
				for sites in arry_all_site:
					sites=str(sites).replace('\'','')
					arry_ac_site_leading.append(ac+'_'+sites)
		arry_leading=[]
		for ac_site in arry_ac_site_leading:
			acm=ac_site[:ac_site.find('_')]
			if not acm in arry_leading:
				arry_leading.append(acm)
		for acb in arry_leading:	#leading匹配完了之后，根据leading匹配的结果，找到leading在pfind鉴定中的所有sameset，再次进行位点匹配
			arry_sameset_ac=[]
			arry_leading_site=[]
			for ac_sitess in arry_ac_site_leading:
				if acb in ac_sitess:
					arry_leading_site.append(ac_sitess)
			file_acs=open('data_protein_group/'+acb)	#这里确定leading的sameset是从pfind的group结果里直接把sameset结果调进来，sameset已经在group收集的时候进行了筛选，全都是正常的ac
			while True:
			...
			print(final_data.replace('\n','')+'\t'+(' ').join(arry_leading_site)+'\t'+(' ').join(arry_sameset_ac),file=file_match_site)
		n+=1
		if n%500==0:
	...
	while True:
		...
		if not arry_data_format_match_site[6][:arry_data_format_match_site[6].find('_')] in arry_leader_ac and arry_data_format_match_site[6][:arry_data_format_match_site[6].find('_')]:
				arry_leader_ac.append(arry_data_format_match_site[6][:arry_data_format_match_site[6].find('_')])
	...
	for leader_ac in arry_leader_ac:
		...
		for ac_x in arry_file_format_match_site:
			...
			if leader_ac+'_' in data_format_match_site:
				arry_data_format_match_site=data_format_match_site.split('\t')
				if arry_data_format_match_site[6][:arry_data_format_match_site[6].find('_')]==leader_ac:
					...
					arry_arry6_data=arry_data_format_match_site[6].split(' ')
					for acs_site in arry_arry6_data:
						acsss=acs_site[acs_site.find('_')+1:]
						if not acsss in arry_all_sitess:
							arry_all_sitess.append(acsss)
			else:
				print(ac_x,file=file_data5,end='')
		file_data5.close()
		file_list=open('int/5_list.txt','at')
		print(leader_ac+'\t'+(' ').join(arry_all_sitess)+'\t'+(' ').join(arry_all_seq),file=file_list)
		file_list.close()
	...
	while True:
		...
		for data11 in arry_data11:
			...
			if '\t'+arry_list_2[0]+'_' in data_final:
				arry_data_final=data_final.split('\t')
				if arry_list_2[0]+'_' in arry_data_final[6]:
	...
	file_data3=open('int/3_confirm_site.txt','rt')	#这个模块是把文件3里的数据完整的拷贝到文件16中，而文件16用于后续的分析工作。
	...
	while True:
		...
		data_final_list_f=data_final_list[:data_final_list.find('\t')]
		arry_ratio=[]
		arry_interference=[]
		arry_identification=[]
		...
		for ac_xx in arry_file_match_site_plus:
			...
			if data_final_list_f==arry_data_match_site[6][:arry_data_match_site[6].find('_')]:
		...
		if calculation=='hl':
			...
			ratio_sd=(sum_ratio_sd/len(arry_ratio))**0.5
		...
		inte_sd=(sum_inte_sd/len(arry_ratio))**0.5
		sum_ide=0
		...
		ide_sd=(sum_ide_sd/len(arry_ratio))**0.5
		print(data_final_list.replace('\n','')+str(round(mideann,5))+'\t'+str(round(ratio_sd,5))+'\t'+str(round(ave_inte,5))+'\t'+str(round(inte_sd,5))+'\t'+str(round(ave_ide,5))+'\t'+str(round(ide_sd,5))+'\t'+str(len(arry_ratio)),file=file_calculating_result)
	...
	file_result=open('results/protein_'+calculation+'_'+file_name.replace('data\\',''),'wt')
	...
	while True:
		...
		print(arry_data10[0]+'\n'+('\n').join(arry_arry3_data10),file=file_list_ac)
	file_data10.close()
	file_list_ac.close()
